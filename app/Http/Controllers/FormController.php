...
class FormController extends Controller
{
    ...
    {
        $form->load(['questions' => function ($q) { $q->orderBy('sort_order'); }, 'organization', 'event']);
        $user = request()->user();
        $isSuperadmin = $user->hasRole('Superadmin');
    ...
    public function update(Request $request, Form $form): RedirectResponse
    {
        ...
        DB::transaction(function () use ($form, $data, $user, $isSuperadmin) {
            $form->update([
                'title'           => $data['title'],
                'description'     => $data['description'] ?? null,
    ...
    public function send(Request $request, Form $form): RedirectResponse
    {
        ...
        foreach ($emails as $email) {
            \Illuminate\Support\Facades\Notification::route('mail', $email)
                ->notify(new \App\Notifications\FormInvitationNotification($form, $recipientName));
        }
