...
function confirmDelete(event) {
    if (confirm('Padam program "' + event.title + '"? Tindakan ini tidak boleh dibatalkan.')) {
        router.delete(route('events.destroy', event.id), {
            preserveScroll: true,
            onSuccess: () => { /* page refreshes via Inertia */ },
        });
    }
}

// ─── Tempoh (Duration) preset ─────────────────────────────────────────────

const duration = ref('khas'); // 'separuh' | 'satu' | 'khas'
...
function applyDurationPreset() {
    ...
    const start = createProgramForm.start_time;
    if (!start) return;
    ...
    if (value === 'separuh') {
        // Sesi pagi → 12 tengah hari; sesi petang → 6 petang
        end.setHours(startDate.getHours() < 12 ? 12 : 18, 0, 0, 0);
    } else if (value === 'satu') {
        // Tempoh penuh sehari → tamat 5 petang (boleh ubah untuk program malam)
        end.setHours(17, 0, 0, 0);
    }

    if (value !== 'khas') {
        createProgramForm.end_time = toDatetimeLocal(end);
    }
}

// Auto-fill masa tamat apabila masa mula dipilih dan tempoh belum ditetapkan.
watch(createProgramForm.start_time, () => {
    if (duration.value !== 'khas' && !createProgramForm.end_time) {
        applyDurationPreset();
    }
});
                    ...
                    <div class="relative aspect-[4/3] overflow-hidden bg-gray-50 shrink-0">
                        ...
                        <div v-if="isSuperadmin" class="absolute top-2 right-2 z-10 flex gap-1">
                            ...
                            <button
                                ...
                                class="flex items-center justify-center w-7 h-7 rounded-lg bg-white/90 backdrop-blur-sm border border-gray-200 text-red-500 shadow-sm hover:bg-red-50 hover:text-red-700 transition text-[10px]"
                                title="Padam"
                            >
                    ...
                    <div class="w-full max-w-2xl rounded-3xl border border-white/50 bg-white/95 shadow-2xl overflow-hidden">
                        ...
                        <form class="space-y-4 p-5" @submit.prevent="submitCreateProgram">
                            ...
                            <div class="space-y-1">
                                ...
                                <select v-model="duration" @change="applyDurationPreset" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm">
