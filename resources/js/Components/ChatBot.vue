...
const STORAGE_POS_KEY = 'mywap_chat_position';
const STORAGE_DISMISS_KEY = 'mywap_chat_dismissed';
const BTN_SIZE = 56;
const BTN_MARGIN = 24;
const PANEL_W = 360;
const PANEL_H = 520;
...
try {
    dismissed.value = sessionStorage.getItem(STORAGE_DISMISS_KEY) === '1';
} catch {}

const panelStyle = computed(() => {
    if (btnX.value === null || btnY.value === null) return {};

    const btnR = btnX.value + BTN_SIZE;
    const btnB = btnY.value + BTN_SIZE;
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    const pad = 16;

    let left = btnR - PANEL_W;
    left = Math.max(pad, Math.min(vw - PANEL_W - pad, left));

    const spaceAbove = btnY.value;
    const spaceBelow = vh - btnB;

    let top;
    if (spaceAbove >= PANEL_H + GAP) {
        top = btnY.value - PANEL_H - GAP;
    ...
    if (savedPos) {
        try {
            ...
            if (pos.x != null && pos.y != null) {
                btnX.value = Math.max(16, Math.min(window.innerWidth - BTN_SIZE - 16, pos.x));
                btnY.value = Math.max(16, Math.min(window.innerHeight - BTN_SIZE - 16, pos.y));
            }
        } catch {}
    }
    if (btnX.value === null) {
        btnX.value = window.innerWidth - BTN_SIZE - BTN_MARGIN;
        btnY.value = window.innerHeight - BTN_SIZE - BTN_MARGIN;
    }
...
function startDrag(e) {
    ...
    const currentX = btnX.value ?? window.innerWidth - BTN_SIZE - BTN_MARGIN;
    const currentY = btnY.value ?? window.innerHeight - BTN_SIZE - BTN_MARGIN;

    dragStarted = false;

    const offX = touch.clientX - currentX;
    const offY = touch.clientY - currentY;

    function onDrag(e) {
        const t = e.touches ? e.touches[0] : e;
        if (Math.abs(t.clientX - offX - currentX) > DRAG_THRESHOLD ||
            Math.abs(t.clientY - offY - currentY) > DRAG_THRESHOLD) {
            dragStarted = true;
        }
        btnX.value = t.clientX - offX;
        btnY.value = t.clientY - offY;
    }
    ...
    function stopDrag() {
        ...
        if (btnX.value !== null) {
            const halfW = BTN_SIZE / 2;
            btnX.value = btnX.value + halfW < window.innerWidth / 2
                ? 16
                : window.innerWidth - BTN_SIZE - 16;
            btnY.value = Math.max(16, Math.min(window.innerHeight - BTN_SIZE - 16, btnY.value));
        }

        localStorage.setItem(STORAGE_POS_KEY, JSON.stringify({ x: btnX.value, y: btnY.value }));
    }
...
function closeChat() {
    ...
    open.value = false;
    try { sessionStorage.setItem(STORAGE_DISMISS_KEY, '1'); } catch {}
}

function restoreChat() {
    dismissed.value = false;
    try { sessionStorage.removeItem(STORAGE_DISMISS_KEY); } catch {}
}
</script>
...
<template>
    ...
    >
        <div
            v-if="open && !dismissed"
            class="fixed z-50 flex w-[360px] max-w-[calc(100vw-48px)] flex-col rounded-2xl border border-gray-100 bg-white shadow-2xl"
    ...
    >
        ...
        >
            ...
            <span
                ...
                class="absolute -top-1 -right-1 flex h-4 w-4 cursor-pointer items-center justify-center rounded-full bg-red-500 text-white shadow-sm transition-colors hover:bg-red-600"
                title="Tutup"
            >
